...
class AppDelegate: NSObject, NSApplicationDelegate {
    ...
    lazy var applicationDocumentsDirectory: NSURL = {
        // The directory the application uses to store the Core Data store file. This code uses a directory named "it.prismatdi.LibraryApp" in the user's Application Support directory.
        let urls = NSFileManager.defaultManager().URLsForDirectory(.ApplicationSupportDirectory, inDomains: .UserDomainMask)
        let appSupportURL = urls[urls.count - 1] as! NSURL
        return appSupportURL.URLByAppendingPathComponent("it.prismatdi.LibraryApp")
    }()
    ...
    lazy var persistentStoreCoordinator: NSPersistentStoreCoordinator? = {
        ...
        if !shouldFail && (error == nil) {
            ...
            let url = self.applicationDocumentsDirectory.URLByAppendingPathComponent("LibraryApp.storedata")
